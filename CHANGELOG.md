...
- Generated Rust / PHP / Swift / Ruby clients throw typed `ApiError` after retries are exhausted (same policy as TS / Python / Go / Java / Kotlin / C#: 400/401/403/404/409/422/429/5xx + timeout/network; truncated body; sent `X-Request-Id`; `RateLimitError` retry-after seconds when present). Stdlib only. Smoke prints `typed-errors-ok`.
- Generated Java / Kotlin / C# clients throw typed `ApiError` after retries are exhausted (same policy as TS / Python / Go: 400/401/403/404/409/422/429/5xx + timeout/network; truncated body; sent `X-Request-Id`; `RateLimitError` retry-after seconds when present). Stdlib only. Smoke prints `typed-errors-ok` (optional live Java 404 when `javac` is on PATH).
- Generated TS / Python / Go clients throw typed `ApiError` after retries are exhausted (subclasses/variants for 400/401/403/404/409/422/429 and 5xx, plus timeout/network). Carries HTTP status, truncated response body, and the sent `X-Request-Id`. `RateLimitError` exposes retry-after seconds when the header was present. Generated stdio MCP servers map upstream failure to a tools/call error payload with status + requestId (auth headers are not copied). Smoke prints `typed-errors-ok`.
