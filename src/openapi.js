...
function emitJavaApiErrors(lines) {
  lines.push(`// Typed errors after retries are exhausted. Body truncated to 2048 chars. requestId is the sent X-Request-Id.`);
  lines.push(`class ApiError extends RuntimeException {`);
  lines.push(`    public final int status;`);
  lines.push(`    public final String body;`);
  lines.push(`    public final String requestId;`);
  lines.push(`    public final Integer retryAfterSeconds;`);
  lines.push(`    public ApiError(String message, int status, String body, String requestId) {`);
  lines.push(`        this(message, status, body, requestId, null);`);
  lines.push(`    }`);
  lines.push(`    public ApiError(String message, int status, String body, String requestId, Integer retryAfterSeconds) {`);
  lines.push(`        super(message == null ? "api error" : message);`);
  lines.push(`        this.status = status;`);
  lines.push(`        this.body = body == null ? "" : (body.length() > 2048 ? body.substring(0, 2048) : body);`);
  lines.push(`        this.requestId = requestId == null ? "" : requestId;`);
  lines.push(`        this.retryAfterSeconds = retryAfterSeconds;`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class BadRequestError extends ApiError {`);
  lines.push(`    public BadRequestError(String body, String requestId) { super("HTTP 400", 400, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class UnauthorizedError extends ApiError {`);
  lines.push(`    public UnauthorizedError(String body, String requestId) { super("HTTP 401", 401, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class ForbiddenError extends ApiError {`);
  lines.push(`    public ForbiddenError(String body, String requestId) { super("HTTP 403", 403, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class NotFoundError extends ApiError {`);
  lines.push(`    public NotFoundError(String body, String requestId) { super("HTTP 404", 404, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class ConflictError extends ApiError {`);
  lines.push(`    public ConflictError(String body, String requestId) { super("HTTP 409", 409, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class UnprocessableEntityError extends ApiError {`);
  lines.push(`    public UnprocessableEntityError(String body, String requestId) { super("HTTP 422", 422, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class RateLimitError extends ApiError {`);
  lines.push(`    public RateLimitError(String body, String requestId, Integer retryAfterSeconds) {`);
  lines.push(`        super("HTTP 429", 429, body, requestId, retryAfterSeconds);`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class ServerError extends ApiError {`);
  lines.push(`    public ServerError(int status, String body, String requestId) {`);
  lines.push(`        super("HTTP " + (status >= 500 ? status : 500), status >= 500 ? status : 500, body, requestId);`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class TimeoutError extends ApiError {`);
  lines.push(`    public TimeoutError(String body, String requestId) { super("timeout", 0, body, requestId); }`);
  lines.push(`}`);
  lines.push(`class NetworkError extends ApiError {`);
  lines.push(`    public NetworkError(String body, String requestId) { super("network", 0, body, requestId); }`);
  lines.push(`}`);
  lines.push(``);
}

function emitKotlinApiErrors(lines) {
  lines.push(`// Typed errors after retries are exhausted. Body truncated to 2048 chars. requestId is the sent X-Request-Id.`);
  lines.push(`open class ApiError(`);
  lines.push(`    message: String,`);
  lines.push(`    val status: Int,`);
  lines.push(`    bodyIn: String?,`);
  lines.push(`    requestIdIn: String?,`);
  lines.push(`    val retryAfterSeconds: Int? = null`);
  lines.push(`) : RuntimeException(message) {`);
  lines.push(`    val body: String = run {`);
  lines.push(`        val s = bodyIn ?: ""`);
  lines.push(`        if (s.length > 2048) s.substring(0, 2048) else s`);
  lines.push(`    }`);
  lines.push(`    val requestId: String = requestIdIn ?: ""`);
  lines.push(`}`);
  lines.push(`class BadRequestError(body: String?, requestId: String?) : ApiError("HTTP 400", 400, body, requestId)`);
  lines.push(`class UnauthorizedError(body: String?, requestId: String?) : ApiError("HTTP 401", 401, body, requestId)`);
  lines.push(`class ForbiddenError(body: String?, requestId: String?) : ApiError("HTTP 403", 403, body, requestId)`);
  lines.push(`class NotFoundError(body: String?, requestId: String?) : ApiError("HTTP 404", 404, body, requestId)`);
  lines.push(`class ConflictError(body: String?, requestId: String?) : ApiError("HTTP 409", 409, body, requestId)`);
  lines.push(`class UnprocessableEntityError(body: String?, requestId: String?) : ApiError("HTTP 422", 422, body, requestId)`);
  lines.push(`class RateLimitError(body: String?, requestId: String?, retryAfterSeconds: Int?) : ApiError("HTTP 429", 429, body, requestId, retryAfterSeconds)`);
  lines.push(`class ServerError(status: Int, body: String?, requestId: String?) : ApiError("HTTP " + (if (status >= 500) status else 500), if (status >= 500) status else 500, body, requestId)`);
  lines.push(`class TimeoutError(body: String?, requestId: String?) : ApiError("timeout", 0, body, requestId)`);
  lines.push(`class NetworkError(body: String?, requestId: String?) : ApiError("network", 0, body, requestId)`);
  lines.push(``);
  lines.push(`fun parseRetryAfterSeconds(raw: String?): Int? {`);
  lines.push(`    if (raw == null) return null`);
  lines.push(`    val t = raw.trim()`);
  lines.push(`    if (t.isEmpty()) return null`);
  lines.push(`    try {`);
  lines.push(`        val sec = t.toDouble()`);
  lines.push(`        if (sec >= 0) return sec.toInt()`);
  lines.push(`    } catch (ignored: NumberFormatException) {`);
  lines.push(`        try {`);
  lines.push(`            val whenAt = java.time.ZonedDateTime.parse(t, java.time.format.DateTimeFormatter.RFC_1123_DATE_TIME)`);
  lines.push(`            var delta = ((whenAt.toInstant().toEpochMilli() - System.currentTimeMillis()) / 1000L).toInt()`);
  lines.push(`            if (delta < 0) delta = 0`);
  ...
  lines.push(`}`);
  lines.push(``);
  lines.push(`fun errorFromStatus(status: Int, body: String?, requestId: String?, retryAfterRaw: String?): ApiError {`);
  lines.push(`    if (status == 400) return BadRequestError(body, requestId)`);
  lines.push(`    if (status == 401) return UnauthorizedError(body, requestId)`);
  lines.push(`    if (status == 403) return ForbiddenError(body, requestId)`);
  lines.push(`    if (status == 404) return NotFoundError(body, requestId)`);
  lines.push(`    if (status == 409) return ConflictError(body, requestId)`);
  lines.push(`    if (status == 422) return UnprocessableEntityError(body, requestId)`);
  lines.push(`    if (status == 429) return RateLimitError(body, requestId, parseRetryAfterSeconds(retryAfterRaw))`);
  lines.push(`    if (status >= 500) return ServerError(status, body, requestId)`);
  lines.push(`    return ApiError("HTTP " + status, status, body, requestId)`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`fun wrapTransportErr(e: Exception, requestId: String?): ApiError {`);
  lines.push(`    val msg = e.message ?: "network"`);
  lines.push(`    if (e is java.net.SocketTimeoutException) return TimeoutError(msg, requestId)`);
  lines.push(`    val lower = msg.lowercase()`);
  lines.push(`    if (lower.contains("timed out") || lower.contains("timeout")) return TimeoutError(msg, requestId)`);
  lines.push(`    return NetworkError(msg, requestId)`);
  lines.push(`}`);
  lines.push(``);
}

function emitCsharpApiErrors(lines) {
  lines.push(`    // Typed errors after retries are exhausted. Body truncated to 2048 chars. RequestId is the sent X-Request-Id.`);
  lines.push(`    public class ApiError : Exception`);
  lines.push(`    {`);
  lines.push(`        public int Status { get; private set; }`);
  lines.push(`        public string Body { get; private set; }`);
  lines.push(`        public string RequestId { get; private set; }`);
  lines.push(`        public int? RetryAfterSeconds { get; private set; }`);
  lines.push(`        public ApiError(string message, int status, string body, string requestId) : this(message, status, body, requestId, null) { }`);
  lines.push(`        public ApiError(string message, int status, string body, string requestId, int? retryAfterSeconds) : base(message ?? "api error")`);
  lines.push(`        {`);
  lines.push(`            this.Status = status;`);
  lines.push(`            string s = body ?? "";`);
  lines.push(`            this.Body = s.Length > 2048 ? s.Substring(0, 2048) : s;`);
  lines.push(`            this.RequestId = requestId ?? "";`);
  lines.push(`            this.RetryAfterSeconds = retryAfterSeconds;`);
  lines.push(`        }`);
  lines.push(`    }`);
  lines.push(`    public class BadRequestError : ApiError { public BadRequestError(string body, string requestId) : base("HTTP 400", 400, body, requestId) { } }`);
  lines.push(`    public class UnauthorizedError : ApiError { public UnauthorizedError(string body, string requestId) : base("HTTP 401", 401, body, requestId) { } }`);
  lines.push(`    public class ForbiddenError : ApiError { public ForbiddenError(string body, string requestId) : base("HTTP 403", 403, body, requestId) { } }`);
  lines.push(`    public class NotFoundError : ApiError { public NotFoundError(string body, string requestId) : base("HTTP 404", 404, body, requestId) { } }`);
  lines.push(`    public class ConflictError : ApiError { public ConflictError(string body, string requestId) : base("HTTP 409", 409, body, requestId) { } }`);
  lines.push(`    public class UnprocessableEntityError : ApiError { public UnprocessableEntityError(string body, string requestId) : base("HTTP 422", 422, body, requestId) { } }`);
  lines.push(`    public class RateLimitError : ApiError { public RateLimitError(string body, string requestId, int? retryAfterSeconds) : base("HTTP 429", 429, body, requestId, retryAfterSeconds) { } }`);
  lines.push(`    public class ServerError : ApiError { public ServerError(int status, string body, string requestId) : base("HTTP " + (status >= 500 ? status : 500), status >= 500 ? status : 500, body, requestId) { } }`);
  lines.push(`    public class TimeoutError : ApiError { public TimeoutError(string body, string requestId) : base("timeout", 0, body, requestId) { } }`);
  lines.push(`    public class NetworkError : ApiError { public NetworkError(string body, string requestId) : base("network", 0, body, requestId) { } }`);
  lines.push(``);
}
...
function emitRustApiErrors(lines) {
  lines.push(`// Typed errors after retries are exhausted. Body truncated to 2048 chars. request_id is the sent X-Request-Id.`);
  lines.push(`const BODY_TRUNCATE: usize = 2048;`);
  lines.push(``);
  lines.push(`fn truncate_body(s: &str) -> String {`);
  lines.push(`    if s.len() <= BODY_TRUNCATE {`);
  lines.push(`        s.to_string()`);
  lines.push(`    } else {`);
  lines.push(`        s[..BODY_TRUNCATE].to_string()`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`fn parse_retry_after_seconds(raw: Option<&str>) -> Option<i64> {`);
  lines.push(`    let t = match raw { Some(v) => v.trim(), None => return None };`);
  lines.push(`    if t.is_empty() {`);
  lines.push(`        return None;`);
  lines.push(`    }`);
  lines.push(`    if let Ok(sec) = t.parse::<f64>() {`);
  lines.push(`        if sec >= 0.0 {`);
  lines.push(`            return Some(sec as i64);`);
  lines.push(`        }`);
  lines.push(`    }`);
  lines.push(`    None`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`fn is_timeout_msg(msg: &str) -> bool {`);
  lines.push(`    let lower = msg.to_ascii_lowercase();`);
  lines.push(`    lower.contains("timed out") || lower.contains("timeout") || lower.contains("deadline")`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`/// Typed HTTP failure after retries are exhausted.`);
  lines.push(`#[derive(Debug, Clone)]`);
  lines.push(`pub struct ApiError {`);
  lines.push(`    pub status: u16,`);
  lines.push(`    pub body: String,`);
  lines.push(`    pub request_id: String,`);
  lines.push(`    pub retry_after_seconds: Option<i64>,`);
  lines.push(`    pub kind: String,`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`impl std::fmt::Display for ApiError {`);
  lines.push(`    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {`);
  lines.push(`        if self.kind == "timeout" {`);
  lines.push(`            write!(f, "timeout request_id={}", self.request_id)`);
  lines.push(`        } else if self.kind == "network" {`);
  lines.push(`            write!(f, "network request_id={}", self.request_id)`);
  lines.push(`        } else {`);
  lines.push(`            write!(f, "HTTP {} request_id={}", self.status, self.request_id)`);
  lines.push(`        }`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`impl std::error::Error for ApiError {}`);
  lines.push(``);
  lines.push(`macro_rules! api_error_newtype {`);
  lines.push(`    ($name:ident) => {`);
  lines.push(`        #[derive(Debug)]`);
  lines.push(`        pub struct $name(pub ApiError);`);
  lines.push(`        impl std::fmt::Display for $name {`);
  lines.push(`            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { self.0.fmt(f) }`);
  lines.push(`        }`);
  lines.push(`        impl std::error::Error for $name {`);
  lines.push(`            fn source(&self) -> Option<&(dyn std::error::Error + 'static)> { Some(&self.0) }`);
  lines.push(`        }`);
  lines.push(`        impl From<$name> for ApiError {`);
  lines.push(`            fn from(e: $name) -> ApiError { e.0 }`);
  lines.push(`        }`);
  lines.push(`    };`);
  lines.push(`}`);
  lines.push(`api_error_newtype!(BadRequestError);`);
  lines.push(`api_error_newtype!(UnauthorizedError);`);
  lines.push(`api_error_newtype!(ForbiddenError);`);
  lines.push(`api_error_newtype!(NotFoundError);`);
  lines.push(`api_error_newtype!(ConflictError);`);
  lines.push(`api_error_newtype!(UnprocessableEntityError);`);
  lines.push(`api_error_newtype!(RateLimitError);`);
  lines.push(`api_error_newtype!(ServerError);`);
  lines.push(`api_error_newtype!(TimeoutError);`);
  lines.push(`api_error_newtype!(NetworkError);`);
  lines.push(``);
  lines.push(`fn wrap_transport_err(msg: String, request_id: String) -> ApiError {`);
  lines.push(`    let timeout = is_timeout_msg(&msg);`);
  lines.push(`    let ae = ApiError {`);
  lines.push(`        status: 0,`);
  lines.push(`        body: truncate_body(&msg),`);
  lines.push(`        request_id,`);
  lines.push(`        retry_after_seconds: None,`);
  lines.push(`        kind: if timeout { "timeout".to_string() } else { "network".to_string() },`);
  lines.push(`    };`);
  lines.push(`    if timeout {`);
  lines.push(`        TimeoutError(ae).into()`);
  lines.push(`    } else {`);
  lines.push(`        NetworkError(ae).into()`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`fn error_from_status(status: u16, body: String, request_id: String, retry_after: Option<&str>) -> ApiError {`);
  lines.push(`    let body = truncate_body(&body);`);
  lines.push(`    let retry_after_seconds = if status == 429 { parse_retry_after_seconds(retry_after) } else { None };`);
  lines.push(`    let ae = ApiError {`);
  lines.push(`        status,`);
  lines.push(`        body,`);
  lines.push(`        request_id,`);
  lines.push(`        retry_after_seconds,`);
  lines.push(`        kind: String::new(),`);
  lines.push(`    };`);
  lines.push(`    match status {`);
  lines.push(`        400 => BadRequestError(ae).into(),`);
  lines.push(`        401 => UnauthorizedError(ae).into(),`);
  lines.push(`        403 => ForbiddenError(ae).into(),`);
  lines.push(`        404 => NotFoundError(ae).into(),`);
  lines.push(`        409 => ConflictError(ae).into(),`);
  lines.push(`        422 => UnprocessableEntityError(ae).into(),`);
  lines.push(`        429 => RateLimitError(ae).into(),`);
  lines.push(`        s if s >= 500 => ServerError(ae).into(),`);
  lines.push(`        _ => ae,`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(``);
...
function emitPhpApiErrors(lines) {
  lines.push(`// Typed errors after retries are exhausted. Body truncated to 2048 chars. requestId is the sent X-Request-Id.`);
  lines.push(`class ApiError extends Exception {`);
  lines.push(`    public $status;`);
  lines.push(`    public $body;`);
  lines.push(`    public $requestId;`);
  lines.push(`    public $retryAfterSeconds;`);
  lines.push(`    public function __construct($message, $status = 0, $body = "", $requestId = "", $retryAfterSeconds = null) {`);
  lines.push(`        parent::__construct($message === null ? "api error" : (string) $message);`);
  lines.push(`        $this->status = intval($status);`);
  lines.push(`        $s = $body === null ? "" : (string) $body;`);
  lines.push(`        $this->body = strlen($s) > 2048 ? substr($s, 0, 2048) : $s;`);
  lines.push(`        $this->requestId = $requestId === null ? "" : (string) $requestId;`);
  lines.push(`        $this->retryAfterSeconds = $retryAfterSeconds;`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class BadRequestError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 400", 400, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class UnauthorizedError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 401", 401, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class ForbiddenError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 403", 403, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class NotFoundError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 404", 404, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class ConflictError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 409", 409, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class UnprocessableEntityError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("HTTP 422", 422, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class RateLimitError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "", $retryAfterSeconds = null) {`);
  lines.push(`        parent::__construct("HTTP 429", 429, $body, $requestId, $retryAfterSeconds);`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class ServerError extends ApiError {`);
  lines.push(`    public function __construct($status = 500, $body = "", $requestId = "") {`);
  lines.push(`        $s = intval($status) >= 500 ? intval($status) : 500;`);
  lines.push(`        parent::__construct("HTTP " . $s, $s, $body, $requestId);`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`class TimeoutError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("timeout", 0, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(`class NetworkError extends ApiError {`);
  lines.push(`    public function __construct($body = "", $requestId = "") { parent::__construct("network", 0, $body, $requestId); }`);
  lines.push(`}`);
  lines.push(``);
}

function emitSwiftApiErrors(lines) {
  lines.push(`// Typed errors after retries are exhausted. Body truncated to 2048 chars. requestId is the sent X-Request-Id.`);
  lines.push(`public class ApiError: Error, CustomStringConvertible {`);
  lines.push(`    public let status: Int`);
  lines.push(`    public let body: String`);
  lines.push(`    public let requestId: String`);
  lines.push(`    public let retryAfterSeconds: Int?`);
  lines.push(`    public let message: String`);
  lines.push(`    public init(message: String, status: Int, body: String?, requestId: String?, retryAfterSeconds: Int? = nil) {`);
  lines.push(`        self.message = message`);
  lines.push(`        self.status = status`);
  lines.push(`        let s = body ?? ""`);
  lines.push(`        self.body = s.count > 2048 ? String(s.prefix(2048)) : s`);
  lines.push(`        self.requestId = requestId ?? ""`);
  lines.push(`        self.retryAfterSeconds = retryAfterSeconds`);
  lines.push(`    }`);
  lines.push(`    public var description: String { return message }`);
  lines.push(`}`);
  lines.push(`public class BadRequestError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 400", status: 400, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class UnauthorizedError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 401", status: 401, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class ForbiddenError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 403", status: 403, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class NotFoundError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 404", status: 404, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class ConflictError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 409", status: 409, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class UnprocessableEntityError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "HTTP 422", status: 422, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class RateLimitError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?, retryAfterSeconds: Int?) {`);
  lines.push(`        super.init(message: "HTTP 429", status: 429, body: body, requestId: requestId, retryAfterSeconds: retryAfterSeconds)`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`public class ServerError: ApiError {`);
  lines.push(`    public init(status: Int, body: String?, requestId: String?) {`);
  lines.push(`        let s = status >= 500 ? status : 500`);
  lines.push(`        super.init(message: "HTTP \\(s)", status: s, body: body, requestId: requestId)`);
  lines.push(`    }`);
  lines.push(`}`);
  lines.push(`public class TimeoutError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "timeout", status: 0, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(`public class NetworkError: ApiError {`);
  lines.push(`    public init(body: String?, requestId: String?) { super.init(message: "network", status: 0, body: body, requestId: requestId) }`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`private func parseRetryAfterSeconds(_ raw: String?) -> Int? {`);
  lines.push(`    guard let raw = raw else { return nil }`);
  lines.push(`    let t = raw.trimmingCharacters(in: .whitespacesAndNewlines)`);
  lines.push(`    if t.isEmpty { return nil }`);
  lines.push(`    if let sec = Double(t), sec >= 0 { return Int(sec) }`);
  lines.push(`    return nil`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`private func errorFromStatus(status: Int, body: String?, requestId: String?, retryAfterRaw: String?) -> ApiError {`);
  lines.push(`    if status == 400 { return BadRequestError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 401 { return UnauthorizedError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 403 { return ForbiddenError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 404 { return NotFoundError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 409 { return ConflictError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 422 { return UnprocessableEntityError(body: body, requestId: requestId) }`);
  lines.push(`    if status == 429 { return RateLimitError(body: body, requestId: requestId, retryAfterSeconds: parseRetryAfterSeconds(retryAfterRaw)) }`);
  lines.push(`    if status >= 500 { return ServerError(status: status, body: body, requestId: requestId) }`);
  lines.push(`    return ApiError(message: "HTTP \\(status)", status: status, body: body, requestId: requestId)`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`private func wrapTransportErr(_ error: Error, requestId: String?) -> ApiError {`);
  lines.push(`    let msg = (error as NSError).localizedDescription`);
  lines.push(`    let lower = msg.lowercased()`);
  lines.push(`    if lower.contains("timed out") || lower.contains("timeout") || (error as NSError).code == NSURLErrorTimedOut {`);
  lines.push(`        return TimeoutError(body: msg, requestId: requestId)`);
  lines.push(`    }`);
  lines.push(`    return NetworkError(body: msg, requestId: requestId)`);
  lines.push(`}`);
  lines.push(``);
}

function emitRubyApiErrors(lines) {
  lines.push(`# Typed errors after retries are exhausted. Body truncated to 2048 chars. request_id is the sent X-Request-Id.`);
  lines.push(`class ApiError < StandardError`);
  lines.push(`  attr_reader :status, :body, :request_id, :retry_after_seconds`);
  lines.push(`  def initialize(message, status: 0, body: "", request_id: "", retry_after_seconds: nil)`);
  lines.push(`    super(message.nil? ? "api error" : message.to_s)`);
  lines.push(`    @status = status.to_i`);
  lines.push(`    s = body.nil? ? "" : body.to_s`);
  lines.push(`    @body = s.length > 2048 ? s[0, 2048] : s`);
  lines.push(`    @request_id = request_id.nil? ? "" : request_id.to_s`);
  lines.push(`    @retry_after_seconds = retry_after_seconds`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class BadRequestError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 400", status: 400, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class UnauthorizedError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 401", status: 401, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class ForbiddenError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 403", status: 403, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class NotFoundError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 404", status: 404, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class ConflictError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 409", status: 409, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class UnprocessableEntityError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("HTTP 422", status: 422, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class RateLimitError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "", retry_after_seconds: nil)`);
  lines.push(`    super("HTTP 429", status: 429, body: body, request_id: request_id, retry_after_seconds: retry_after_seconds)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class ServerError < ApiError`);
  lines.push(`  def initialize(status:, body: "", request_id: "")`);
  lines.push(`    s = status.to_i >= 500 ? status.to_i : 500`);
  lines.push(`    super("HTTP #{s}", status: s, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class TimeoutError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("timeout", status: 0, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(`class NetworkError < ApiError`);
  lines.push(`  def initialize(body: "", request_id: "")`);
  lines.push(`    super("network", status: 0, body: body, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(``);
}
...
export function generateJavaClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`import java.net.SocketTimeoutException;`);
  ...
  lines.push(`    }`);
  lines.push(``);
  lines.push(`    private static Integer parseRetryAfterSeconds(String raw) {`);
  lines.push(`        if (raw == null) {`);
  lines.push(`            return null;`);
  lines.push(`        }`);
  lines.push(`        raw = raw.trim();`);
  lines.push(`        if (raw.length() == 0) {`);
  lines.push(`            return null;`);
  lines.push(`        }`);
  lines.push(`        try {`);
  lines.push(`            double sec = Double.parseDouble(raw);`);
  lines.push(`            if (sec >= 0) {`);
  lines.push(`                return Integer.valueOf((int) sec);`);
  lines.push(`            }`);
  lines.push(`        } catch (NumberFormatException ignored) {`);
  lines.push(`            try {`);
  lines.push(`                java.time.ZonedDateTime when = java.time.ZonedDateTime.parse(raw, java.time.format.DateTimeFormatter.RFC_1123_DATE_TIME);`);
  lines.push(`                long delta = (when.toInstant().toEpochMilli() - System.currentTimeMillis()) / 1000L;`);
  lines.push(`                if (delta < 0) {`);
  lines.push(`                    delta = 0;`);
  lines.push(`                }`);
  lines.push(`                return Integer.valueOf((int) delta);`);
  lines.push(`            } catch (Exception ignored2) {`);
  lines.push(`            }`);
  lines.push(`        }`);
  lines.push(`        return null;`);
  lines.push(`    }`);
  lines.push(``);
  lines.push(`    private static ApiError errorFromStatus(int status, String body, String requestId, String retryAfterRaw) {`);
  lines.push(`        if (status == 400) {`);
  lines.push(`            return new BadRequestError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 401) {`);
  lines.push(`            return new UnauthorizedError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 403) {`);
  lines.push(`            return new ForbiddenError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 404) {`);
  lines.push(`            return new NotFoundError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 409) {`);
  lines.push(`            return new ConflictError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 422) {`);
  lines.push(`            return new UnprocessableEntityError(body, requestId);`);
  lines.push(`        }`);
  lines.push(`        if (status == 429) {`);
  lines.push(`            return new RateLimitError(body, requestId, parseRetryAfterSeconds(retryAfterRaw));`);
  lines.push(`        }`);
  lines.push(`        if (status >= 500) {`);
  lines.push(`            return new ServerError(status, body, requestId);`);
  lines.push(`        }`);
  lines.push(`        return new ApiError("HTTP " + status, status, body, requestId);`);
  lines.push(`    }`);
  lines.push(``);
  lines.push(`    private static ApiError wrapTransportErr(Exception e, String requestId) {`);
  lines.push(`        String msg = e == null || e.getMessage() == null ? "network" : e.getMessage();`);
  lines.push(`        if (e instanceof SocketTimeoutException) {`);
  lines.push(`            return new TimeoutError(msg, requestId);`);
  lines.push(`        }`);
  lines.push(`        String lower = msg.toLowerCase();`);
  lines.push(`        if (lower.contains("timed out") || lower.contains("timeout")) {`);
  lines.push(`            return new TimeoutError(msg, requestId);`);
  lines.push(`        }`);
  lines.push(`        return new NetworkError(msg, requestId);`);
  lines.push(`    }`);
  lines.push(``);
  const reqSig = auth.any
  ...
  lines.push(`                    attemptRid = rid;`);
  lines.push(`                } else {`);
  lines.push(`                    String existing = conn.getRequestProperty("X-Request-Id");`);
  lines.push(`                    attemptRid = existing == null ? "" : existing;`);
  lines.push(`                }`);
  lines.push(`                lastRid = attemptRid;`);
  ...
  lines.push(`                    continue;`);
  lines.push(`                }`);
  lines.push(`                throw errorFromStatus(code, text, attemptRid, conn.getHeaderField("Retry-After"));`);
  lines.push(`            } catch (ApiError e) {`);
  lines.push(`                throw e;`);
  ...
  lines.push(`                last = e;`);
  lines.push(`                if (attempt >= 2) {`);
  lines.push(`                    throw wrapTransportErr(e, lastRid);`);
  lines.push(`                }`);
  ...
  lines.push(`            throw wrapTransportErr(last, lastRid);`);
  lines.push(`        }`);
  lines.push(`        throw new NetworkError(method + " " + path + " -> network", lastRid);`);
  lines.push(`    }`);
  lines.push(``);
...
export function generateRustClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`// Typed errors after retries are exhausted: ApiError + 400/401/403/404/409/422/429/5xx + TimeoutError/NetworkError. Body truncated to 2048 chars. request_id is the sent X-Request-Id. RateLimitError.retry_after_seconds when Retry-After was present.`);
  ...
  lines.push(``);
  const reqSig = auth.any
    ? `    fn request(&self, method: &str, rel_path: &str, body: Option<&str>, auth_bearer: bool, auth_headers: &[&str], auth_query: &[&str]) -> Result<String, ApiError> {`
    : `    fn request(&self, method: &str, rel_path: &str, body: Option<&str>) -> Result<String, ApiError> {`;
  lines.push(reqSig);
  ...
  lines.push(`            }`);
  lines.push(`        };`);
  lines.push(`        let mut last = wrap_transport_err(String::from("network"), String::new());`);
  ...
  lines.push(onceCall);
  lines.push(`                Ok((code, retry_after, body_text, rid)) => {`);
  ...
  lines.push(`                        continue;`);
  lines.push(`                    }`);
  lines.push(`                    return Err(error_from_status(code, body_text, rid, retry_after.as_deref()));`);
  lines.push(`                }`);
  lines.push(`                Err((msg, rid)) => {`);
  lines.push(`                    last = wrap_transport_err(msg, rid);`);
  lines.push(`                    if attempt >= 2 {`);
  ...
  const onceSig = auth.any
    ? `    fn request_once(&self, method: &str, rel_path: &str, body: Option<&str>, ${bearerP}, ${headersP}, ${queryP}, op_idem: &str) -> Result<(u16, Option<String>, String, String), (String, String)> {`
    : `    fn request_once(&self, method: &str, rel_path: &str, body: Option<&str>, op_idem: &str) -> Result<(u16, Option<String>, String, String), (String, String)> {`;
  lines.push(onceSig);
  ...
  lines.push(`            if !t.is_empty() { t.to_string() } else { new_request_id() }`);
  lines.push(`        };`);
  lines.push(`        let map_err = |e: String| -> (String, String) { (e, rid.clone()) };`);
  lines.push(`        let (host, port, base_path) = match parse_http_base(&self.base_url) { Ok(v) => v, Err(e) => return Err(map_err(e)) };`);
  ...
  lines.push(`            format!("{}:{}", host, port)`);
  lines.push(`        };`);
  lines.push(`        let mut addrs = match addr.to_socket_addrs() { Ok(a) => a, Err(e) => return Err(map_err(e.to_string())) };`);
  lines.push(`        let sock = match addrs.next() { Some(s) => s, None => return Err(map_err(format!("no address for {}", addr))) };`);
  lines.push(`        let mut stream = match TcpStream::connect_timeout(&sock, timeout) { Ok(s) => s, Err(e) => return Err(map_err(e.to_string())) };`);
  lines.push(`        if let Err(e) = stream.set_read_timeout(Some(timeout)) { return Err(map_err(e.to_string())); }`);
  lines.push(`        if let Err(e) = stream.set_write_timeout(Some(timeout)) { return Err(map_err(e.to_string())); }`);
  ...
  lines.push(`        }`);
  lines.push(`        if let Err(e) = stream.write_all(req.as_bytes()) { return Err(map_err(e.to_string())); }`);
  lines.push(`        let mut buf = Vec::new();`);
  lines.push(`        if let Err(e) = stream.read_to_end(&mut buf) { return Err(map_err(e.to_string())); }`);
  ...
  lines.push(`            None => return Err(map_err("invalid HTTP response".into())),`);
  lines.push(`        };`);
  ...
  for (const op of ops) {
    ...
    const summary = String(op.summary || `${op.method} ${op.path}`)
      ...
      .replace(/\r?\n/g, " ");
    lines.push(`    /// ${fn} ${summary} (operationId: ${op.operationId})`);
    lines.push(`    pub fn ${fn}(&self, args: HashMap<String, String>) -> Result<String, ApiError> {`);
...
export function generateCsharpClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`        }`);
  lines.push(``);
  lines.push(`        private static int? ParseRetryAfterSeconds(HttpResponseMessage res)`);
  lines.push(`        {`);
  lines.push(`            if (res == null || res.Headers.RetryAfter == null)`);
  lines.push(`            {`);
  lines.push(`                return null;`);
  ...
  lines.push(`            if (res.Headers.RetryAfter.Date != null)`);
  lines.push(`            {`);
  lines.push(`                double delta = (res.Headers.RetryAfter.Date.Value - DateTimeOffset.UtcNow).TotalSeconds;`);
  lines.push(`                if (delta < 0)`);
  lines.push(`                {`);
  lines.push(`                    delta = 0;`);
  lines.push(`                }`);
  ...
  lines.push(`        }`);
  lines.push(``);
  lines.push(`        private static ApiError ErrorFromStatus(int status, string body, string requestId, int? retryAfterSeconds)`);
  lines.push(`        {`);
  lines.push(`            if (status == 400) return new BadRequestError(body, requestId);`);
  lines.push(`            if (status == 401) return new UnauthorizedError(body, requestId);`);
  lines.push(`            if (status == 403) return new ForbiddenError(body, requestId);`);
  ...
  lines.push(`            if (status == 409) return new ConflictError(body, requestId);`);
  lines.push(`            if (status == 422) return new UnprocessableEntityError(body, requestId);`);
  lines.push(`            if (status == 429) return new RateLimitError(body, requestId, retryAfterSeconds);`);
  lines.push(`            if (status >= 500) return new ServerError(status, body, requestId);`);
  lines.push(`            return new ApiError("HTTP " + status, status, body, requestId);`);
  lines.push(`        }`);
  lines.push(``);
  lines.push(`        private static ApiError WrapTransportErr(Exception e, string requestId)`);
  lines.push(`        {`);
  lines.push(`            string msg = e == null || e.Message == null ? "network" : e.Message;`);
  lines.push(`            if (e is System.Threading.Tasks.TaskCanceledException || e is OperationCanceledException || e is TimeoutException)`);
  lines.push(`            {`);
  lines.push(`                return new TimeoutError(msg, requestId);`);
  lines.push(`            }`);
  lines.push(`            string lower = msg.ToLowerInvariant();`);
  lines.push(`            if (lower.Contains("timed out") || lower.Contains("timeout") || lower.Contains("canceled") || lower.Contains("cancelled"))`);
  lines.push(`            {`);
  lines.push(`                return new TimeoutError(msg, requestId);`);
  lines.push(`            }`);
  lines.push(`            return new NetworkError(msg, requestId);`);
  ...
  lines.push(`                else`);
  lines.push(`                {`);
  lines.push(`                    attemptRid = string.Join(",", req.Headers.GetValues("X-Request-Id"));`);
  lines.push(`                }`);
  lines.push(`                lastRid = attemptRid;`);
  ...
  lines.push(`                                continue;`);
  lines.push(`                            }`);
  lines.push(`                            throw ErrorFromStatus(code, text, attemptRid, ParseRetryAfterSeconds(res));`);
  lines.push(`                        }`);
  lines.push(`                    }`);
  lines.push(`                }`);
  lines.push(`                catch (ApiError)`);
  lines.push(`                {`);
  lines.push(`                    throw;`);
  lines.push(`                }`);
  lines.push(`                catch (Exception e)`);
...
export function generateKotlinClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`                    attemptRid = rid`);
  lines.push(`                } else {`);
  lines.push(`                    val existing = conn.getRequestProperty("X-Request-Id")`);
  lines.push(`                    attemptRid = if (existing == null) "" else existing`);
  lines.push(`                }`);
  lines.push(`                lastRid = attemptRid`);
  ...
  lines.push(`            throw wrapTransportErr(last!!, lastRid)`);
  lines.push(`        }`);
  lines.push(`        throw NetworkError(method + " " + path + " -> network", lastRid)`);
  lines.push(`    }`);
  lines.push(``);
...
export function generateSwiftClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`                    throw NetworkError(body: "invalid URL", requestId: lastRid)`);
  lines.push(`                }`);
  ...
  lines.push(`                    attemptRid = rid`);
  lines.push(`                } else {`);
  lines.push(`                    attemptRid = req.value(forHTTPHeaderField: "X-Request-Id") ?? ""`);
  lines.push(`                }`);
  lines.push(`                lastRid = attemptRid`);
  ...
  lines.push(`                    continue`);
  lines.push(`                }`);
  lines.push(`                throw errorFromStatus(status: code, body: String(data: resultData ?? Data(), encoding: .utf8), requestId: attemptRid, retryAfterRaw: (resultResponse as? HTTPURLResponse)?.value(forHTTPHeaderField: "Retry-After"))`);
  lines.push(`            } catch let apiErr as ApiError {`);
  lines.push(`                throw apiErr`);
  lines.push(`            } catch {`);
  lines.push(`                lastError = error`);
  lines.push(`                if attempt >= 2 {`);
  lines.push(`                    throw wrapTransportErr(error, requestId: lastRid)`);
  lines.push(`                }`);
  lines.push(`                Thread.sleep(forTimeInterval: Double(100 * (1 << attempt)) / 1000.0)`);
  lines.push(`            }`);
  lines.push(`        }`);
  lines.push(`        if let lastError = lastError {`);
  lines.push(`            throw wrapTransportErr(lastError, requestId: lastRid)`);
  lines.push(`        }`);
  lines.push(`        throw NetworkError(body: method + " " + path + " -> network", requestId: lastRid)`);
  lines.push(`    }`);
  lines.push(``);
...
export function generateRubyClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  emitRubyApiErrors(lines);
  lines.push(`def parse_retry_after_seconds(raw)`);
  lines.push(`  return nil if raw.nil? || raw.to_s.strip.empty?`);
  lines.push(`  t = raw.to_s.strip`);
  lines.push(`  begin`);
  lines.push(`    sec = Float(t)`);
  lines.push(`    return sec.to_i if sec >= 0`);
  lines.push(`  rescue ArgumentError`);
  lines.push(`    begin`);
  lines.push(`      when_t = Time.httpdate(t)`);
  lines.push(`      n = (when_t - Time.now).to_i`);
  lines.push(`      return n < 0 ? 0 : n`);
  lines.push(`    rescue ArgumentError`);
  lines.push(`    end`);
  lines.push(`  end`);
  lines.push(`  nil`);
  lines.push(`end`);
  lines.push(``);
  lines.push(`def error_from_status(status, body, request_id, retry_after_raw = nil)`);
  lines.push(`  case status`);
  lines.push(`  when 400 then BadRequestError.new(body: body, request_id: request_id)`);
  lines.push(`  when 401 then UnauthorizedError.new(body: body, request_id: request_id)`);
  lines.push(`  when 403 then ForbiddenError.new(body: body, request_id: request_id)`);
  lines.push(`  when 404 then NotFoundError.new(body: body, request_id: request_id)`);
  lines.push(`  when 409 then ConflictError.new(body: body, request_id: request_id)`);
  lines.push(`  when 422 then UnprocessableEntityError.new(body: body, request_id: request_id)`);
  lines.push(`  when 429 then RateLimitError.new(body: body, request_id: request_id, retry_after_seconds: parse_retry_after_seconds(retry_after_raw))`);
  lines.push(`  else`);
  lines.push(`    if status >= 500`);
  lines.push(`      ServerError.new(status: status, body: body, request_id: request_id)`);
  lines.push(`    else`);
  lines.push(`      ApiError.new("HTTP #{status}", status: status, body: body, request_id: request_id)`);
  lines.push(`    end`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(``);
  lines.push(`def wrap_transport_err(err, request_id)`);
  lines.push(`  msg = err.respond_to?(:message) ? err.message.to_s : err.to_s`);
  lines.push(`  if err.is_a?(Timeout::Error) || msg.downcase.include?("timeout") || msg.downcase.include?("timed out")`);
  lines.push(`    TimeoutError.new(body: msg, request_id: request_id)`);
  lines.push(`  else`);
  lines.push(`    NetworkError.new(body: msg, request_id: request_id)`);
  lines.push(`  end`);
  lines.push(`end`);
  lines.push(``);
  lines.push(`class Client`);
  ...
  lines.push(`          last_rid = rid`);
  lines.push(`        else`);
  lines.push(`          last_rid = req["X-Request-Id"].to_s`);
  lines.push(`        end`);
  ...
  lines.push(`          next`);
  lines.push(`        end`);
  lines.push(`        raise error_from_status(code, res.body.to_s, last_rid, res["Retry-After"])`);
  ...
  lines.push(`        last = e`);
  lines.push(`        raise wrap_transport_err(e, last_rid) if attempt >= 2`);
  lines.push(`        sleep(0.1 * (2 ** attempt))`);
  lines.push(`      end`);
  lines.push(`    end`);
  lines.push(`    raise wrap_transport_err(last, last_rid) if last`);
  lines.push(`    raise NetworkError.new(body: method.to_s + " " + path.to_s + " -> network", request_id: last_rid)`);
  lines.push(`  end`);
  lines.push(``);
...
export function generatePhpClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push("    }");
  lines.push("");
  lines.push("    private static function parseRetryAfterSeconds($raw) {");
  lines.push("        if ($raw === null) { return null; }");
  lines.push("        $t = trim((string) $raw);");
  lines.push("        if ($t === \"\") { return null; }");
  lines.push("        if (is_numeric($t)) {");
  lines.push("            $sec = floatval($t);");
  lines.push("            if ($sec >= 0) { return intval($sec); }");
  lines.push("        }");
  lines.push("        $when = strtotime($t);");
  lines.push("        if ($when !== false) {");
  lines.push("            $delta = $when - time();");
  lines.push("            return $delta < 0 ? 0 : intval($delta);");
  lines.push("        }");
  lines.push("        return null;");
  lines.push("    }");
  lines.push("");
  lines.push("    private static function errorFromStatus($status, $body, $requestId, $retryAfterRaw = null) {");
  lines.push("        if ($status === 400) { return new BadRequestError($body, $requestId); }");
  lines.push("        if ($status === 401) { return new UnauthorizedError($body, $requestId); }");
  lines.push("        if ($status === 403) { return new ForbiddenError($body, $requestId); }");
  lines.push("        if ($status === 404) { return new NotFoundError($body, $requestId); }");
  lines.push("        if ($status === 409) { return new ConflictError($body, $requestId); }");
  lines.push("        if ($status === 422) { return new UnprocessableEntityError($body, $requestId); }");
  lines.push("        if ($status === 429) { return new RateLimitError($body, $requestId, self::parseRetryAfterSeconds($retryAfterRaw)); }");
  lines.push("        if ($status >= 500) { return new ServerError($status, $body, $requestId); }");
  lines.push("        return new ApiError(\"HTTP \" . $status, $status, $body, $requestId);");
  lines.push("    }");
  lines.push("");
  lines.push("    private static function wrapTransportErr($msg, $requestId) {");
  lines.push("        $m = $msg === null ? \"network\" : (string) $msg;");
  lines.push("        $lower = strtolower($m);");
  lines.push("        if (strpos($lower, \"timed out\") !== false || strpos($lower, \"timeout\") !== false) {");
  lines.push("            return new TimeoutError($m, $requestId);");
  lines.push("        }");
  lines.push("        return new NetworkError($m, $requestId);");
  lines.push("    }");
  lines.push("");
  ...
  lines.push("                $last = self::wrapTransportErr((string) $method . \" \" . (string) $path . \" -> open failed\", $lastRid);");
  lines.push("                if ($attempt >= 2) {");
  ...
  lines.push("                continue;");
  lines.push("            }");
  lines.push("            $retryAfterRaw = null;");
  lines.push("            foreach ($wrapper as $line) {");
  lines.push("                if (is_string($line) && stripos($line, \"Retry-After:\") === 0) {");
  lines.push("                    $retryAfterRaw = trim(substr($line, 12));");
  lines.push("                }");
  lines.push("            }");
  lines.push("            throw self::errorFromStatus($code, $text === false ? \"\" : (string) $text, $lastRid, $retryAfterRaw);");
  lines.push("        }");
  lines.push("        if ($last !== null) {");
  lines.push("            throw $last;");
  lines.push("        }");
  lines.push("        throw new NetworkError((string) $method . \" \" . (string) $path . \" -> network\", $lastRid);");
  lines.push("    }");
  lines.push("");
