...
export function generateTsClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`export interface ClientOptions { baseUrl?: string; fetchImpl?: typeof fetch }`);
  ...
  lines.push(`    const res = await f(baseUrl + path, {`);
  lines.push(`      method,`);
...
export function generatePyClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(``);
  lines.push(`    def __init__(self, base_url: str = "", opener: Any = None) -> None:`);
  ...
  lines.push(`        try:`);
  lines.push(`            with open_url(req) as res:`);
  ...
  lines.push(`            raise RuntimeError(f"{method} {path} -> {e.code}") from e`);
  lines.push(``);
  for (const op of ops) {
    const fn = op.operationId;
  ...
  lines.push(`    return Client(base_url=base_url, opener=opener)`);
  lines.push(``);
  return lines.join("\n");
}
...
export function generateGoClient(ops, title = "GeneratedClient", opts = {}) {
  ...
  lines.push(`\t\t}`);
  lines.push(`\t\trdr = bytes.NewReader(b)`);
  lines.push(`\t}`);
  lines.push(`\treq, err := http.NewRequest(method, c.BaseURL+path, rdr)`);
  lines.push(`\tif err != nil {`);
  lines.push(`\t\treturn nil, err`);
  lines.push(`\t}`);
  lines.push(`\tif body != nil {`);
